...
export const PUT = async (req: NextRequest, { params }: { params: { categoryId: string; } }) => {
    ...
    try {
      const category = await prisma.catetory.findFirst({ where: { id } });
      ...
      const res = await prisma.catetory.update({
        where: {
          id
        },
        data: {
  ...
  export const DELETE = async (req: NextRequest, { params }: { params: { categoryId: string; } }) => {
    ...
    try {
        ...
        const parent = await prisma.catetory.findFirst({where: { id }  })
        if (parent == null) {
            return NextResponse.json({
              code: "CATEGORY_NOT_EXIST",
              message: "Category không tồn tại"
        ...
        const res = await prisma.catetory.delete({
          where: {
            id
          }
        })
