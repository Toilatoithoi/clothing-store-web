  ...
  return (
    <div className='w-full h-full bg-slate-100 flex'>
      <div className='w-[30rem] h-full border-r flex flex-col gap-[0.4rem] border-r-gray-200  bg-white ' >
        <div className='h-[7.6rem] font-bold text-[2.4rem] text-center border-b border-b-gray-200'>
          360 CMS
        </div>
        <div className='flex flex-col '>
          {cmsSideBar.map((item, idx) =>
            <Link href={item.route} key={idx} className={`h-[4rem] ${activeRoute?.route === item.route ? 'bg-slate-200 text-black' : ''} hover:bg-slate-200 flex items-center px-[1.6rem] cursor-pointer !font-bold border-b border-b-gray-200`}>
              <div>{item.label}</div>
            </Link>)}
        </div>
      </div>
